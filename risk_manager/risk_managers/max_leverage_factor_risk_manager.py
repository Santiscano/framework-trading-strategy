...
from utils.utils import Utils, print_warning
...
class MaxLeverageFactorRiskManager(IRiskManager):
    ...
    def _check_expected_new_position_is_compliant_with_max_leverage_factor(self, sizing_event: SizingEvent,
        ...
        if abs(new_leverage_factor) <= self.max_leverage_factor:
            ...
            print_warning(f"RISK MGMT: La posición objetivo {sizing_event.signal} {sizing_event.volume} implica un Leverage Factor de {abs(new_leverage_factor):.2f}, que supera el máx. de {self.max_leverage_factor}")
            return False



    def assess_order(self, sizing_event: SizingEvent, current_positions_value_acc_ccy: float, new_position_value_acc_ccy: float):
        """
