...
class SignalGenerator(ISignalGenerator):
    ...
    def _get_signal_generator_method(self, signal_props: BaseSignalProps) -> ISignalGenerator:
        ...
        if isinstance (signal_props, MACrossoverProps):
            return SignalMACrossover(properties=signal_props, events_queue=self.events_queue, data_provider=self.DATA_PROVIDER)
        elif isinstance (signal_props, RSIProps):
            return SignalRSI(properties=signal_props)
        else:
            print_exception(f"ERROR: Estrategia desconocida: {signal_props}")
    
    def generate_signal(self, data_event: DataEvent):
        """
        ...
        signal_event = self.signal_generator_method.generate_signal(data_event, 
                                                                    self.DATA_PROVIDER, 
                                                                    self.PORTFOLIO
                                                                    # self.ORDER_EXECUTOR
                                                                    )
