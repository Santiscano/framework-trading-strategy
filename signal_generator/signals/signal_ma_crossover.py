...
class SignalMACrossover(ISignalGenerator):
    ...
    def __init__(self, properties: MACrossoverProps):
        ...
        if self.fast_period >= self.slow_period:
            print_exception(f"ERROR: el perdiodo rápido ({self.fast_period}) es mayor o igual al periodo lento ({self.slow_period}) para el cálculo de las medias móviles.")
    ...
    def generate_signal(self, 
        ...
        if open_positions['LONG'] == 0 and fast_ma > slow_ma:
            if open_positions['SHORT'] > 0:
                ...
                pass # order_executor.close_strategy_short_positions_by_symbol(symbol)
            signal = "BUY"
        
        # Detectar una señal de venta
        elif open_positions['SHORT'] == 0 and slow_ma > fast_ma:
            if open_positions['LONG'] > 0:
                # Tenemos señal de venta, pero tenemos posición de compra. Debemos cerrar la compra ANTES de abrir la venta.
                pass # order_executor.close_strategy_long_positions_by_symbol(symbol)
            signal = "SELL"
